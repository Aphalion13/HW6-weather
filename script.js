...
// Separates the search term using the commas (the user must follow the example shown in the place holder)
    console.log(searchArray);
    if(searchArray.length == 3){ 
//If the array is length 3 the user entered city, state and country
        for (let i=0; i<searchArray.length;i++){
            searchArray[i]=searchArray[i].trim();   
//deletes spaces from the beggining and end of the values of the array              
        }
        console.log(searchArray);
...
//replaces the space, in two word cities, by '%20'
...
// otherwise, if the array is length 2, the user enter City and Country
        for (let i=0; i<searchArray.length;i++){
            searchArray[i]=searchArray[i].trim();                
        }
        console.log(searchArray);
...
//if it's more than 3 or less than 2, asks to reenter the info
        window.alert('You need to introduce a City, State (if US) and country');
        return;
    }  
}
...
function generateButton(event,array){
    ...
    switch(event){
        case 1: 
//occurs when the method is called from the historic searches stored in local memory
            ...
            break;
        case 2: 
//occurs when the method is called from a new search
            divEl= document.createElement('div');
            divEl.classList.add('btn-group', 'me-2');
            divEl.innerHTML=
                `
